...
LP3AIK.MobileMenu = {
  init: function () {
    ...
    var iconMap = {
      ...
      'Informasi': 'fa-rectangle-list',
      'Program': 'fa-briefcase',
      'Galeri': 'fa-images',
      'Unduhan': 'fa-download',
      'Hubungi': 'fa-phone',
      'Profil': 'fa-building',
      'Pengajar': 'fa-chalkboard-user',
      ...
      'SPMB': 'fa-user-plus',
      'Rekrutmen': 'fa-users-gear',
      'FAQ': 'fa-circle-question'
    };

    // Target ALL navigation links globally (Desktop site-header + Mobile Drawer)
    document.querySelectorAll('.mobile-nav-list li a, .primary-nav li a').forEach(function (link) {
      var text = link.textContent.trim();
      // Fix: Exclude chevrons/arrows from this check so parent menus (which have arrows) can still receive custom leading icons!
      var existingIcon = link.querySelector('i:not(.fa-chevron-down):not(.fa-chevron-up):not(.fa-angle-down):not(.fa-angle-up)');
      
      if (!existingIcon) {
        for (var key in iconMap) {
          if (text.toLowerCase().includes(key.toLowerCase())) {
            var icon = document.createElement('i');
            icon.className = 'fa-solid ' + iconMap[key];
            link.prepend(icon);
            break;
          }
        }
    ...
    mobileDrawer.querySelectorAll('.menu-item-has-children > a').forEach(function (link) {
      // Relocate ANY existing chevron generated by WP so it flows to the absolute far-right end
      var existingChevron = link.querySelector('.fa-chevron-down, .fa-angle-down, .submenu-arrow');
      
      if (existingChevron) {
        // By moving it to the absolute end, our CSS 'margin-left: auto' pushes it perfectly to the right!
        link.appendChild(existingChevron);
      } else {
        // Inject arrow only if no other chevron indicator exists in plain HTML
        var hasPlainChevron = link.innerHTML.includes('fa-chevron-down') || link.innerHTML.includes('fa-angle-down');
        if (!hasPlainChevron) {
